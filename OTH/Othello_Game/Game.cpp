...
stringstream coordinates;
  ...
  whiteScore=2;
   ...
   timer_pic = new Gtk:: Image("img/timer.png");
    ...
    time_label->get_style_context()->add_provider(css_provider, GTK_STYLE_PROVIDER_PRIORITY_USER);
  ...
  for(int i=0;i<8;i++)
    ...
    hBox3->pack_start(*timer_pic);
    hBox3->pack_start(*time_label);
...
Game::Game(int Type_game,int levelType,int board_t)
{
  ...
  whiteScore=2;
    ...
    Stop=Gtk::manage(new Gtk::Button("Stop"));
    Restart=Gtk::manage(new Gtk::Button("Restart"));
    ...
    Stop->signal_clicked().connect(sigc::mem_fun(*this,&Game::On_Arreter_Clicked));
    Restart->signal_clicked().connect(sigc::mem_fun(*this,&Game::On_Relancer_Clicked));
    ...
    stop->signal_activate().connect(sigc::mem_fun(*this,&Game::On_Arreter_Clicked));
    game->append(*stop);
   ...
   Glib::signal_timeout().connect( sigc::mem_fun(*this, &Game::on_timer), 1000 );
...
void Game::flipTiles(Board *board, int x, int y, int deltaX, int deltaY, int myPiece, int opponentPiece,bool status)
{
  ...
  int i=x+deltaX,j=y+deltaY;
   ...
   if(myPiece==-1)
	...
	second =0 ;
	time_label->set_text (" 0 s ");
	Glib::signal_timeout().connect( sigc::mem_fun(*this, &Game::on_timer), 10000 );
    }
void Game::Menu_Quit()
{
